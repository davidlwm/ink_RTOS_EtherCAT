 ...
 *  Modified for STM32F4 HAL integration
 * --------------------------------------------------------------------------
 */
 ...
 * @return HAL status
 */
HAL_StatusTypeDef mcp23017_readPort(MCP23017_HandleTypeDef *hdev, MCP23017Port_t port, uint8_t *data)
{
	uint8_t regAddr = (port == MCP23017Port_A) ? MCP23017_GPIOA : MCP23017_GPIOB;
	return mcp23017_readRegister(hdev, regAddr, data);
}
 ...
 * @return HAL status
 */
HAL_StatusTypeDef mcp23017_writePort(MCP23017_HandleTypeDef *hdev, MCP23017Port_t port, uint8_t data)
{
	uint8_t regAddr = (port == MCP23017Port_A) ? MCP23017_GPIOA : MCP23017_GPIOB;
	return mcp23017_writeRegister(hdev, regAddr, data);
}
